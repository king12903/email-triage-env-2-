...
def get_tasks():
    return {
        "tasks": [
            {"id": "easy", "name": "Spam Detection", "description": "Classify emails as spam or not-spam.", "difficulty": "easy",
             "action_schema": {"priority": "high|medium|low", "category": "billing|support|spam|inquiry", "should_reply": "boolean", "reply_text": "string (optional)"}},
            {"id": "medium", "name": "Priority & Category Triage", "description": "Assign correct priority and category.", "difficulty": "medium",
             "action_schema": {"priority": "high|medium|low", "category": "billing|support|spam|inquiry", "should_reply": "boolean", "reply_text": "string (optional)"}},
            {"id": "hard", "name": "Full Email Triage", "description": "Complete triage with reply.", "difficulty": "hard",
             "action_schema": {"priority": "high|medium|low", "category": "billing|support|spam|inquiry", "should_reply": "boolean", "reply_text": "string (optional)"}},
        ]
    }
...
async def grader(request: Request):
    ...
    return {"task_id": task_id, "score": final_score, "cumulative_reward": s.cumulative_reward, "steps_completed": s.current_step, "done": s.done}
...
def baseline():
    ...
    for task_id in ["easy", "medium", "hard"]:
        ...
        while True:
            ...
            elif any(w in subject for w in ["bug", "error", "broken", "fix"]):
                priority, category, should_reply = "high", "support", True
            elif any(w in subject for w in ["question", "pricing", "upgrade"]):
            ...
            if done:
                break
        results[task_id] = {"score": round(total_reward / steps, 3), "total_reward": round(total_reward, 3), "steps": steps}
